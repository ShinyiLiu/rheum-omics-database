...
from scripts.build_ra_geo_ledger import (
    ...
    extract_source_pubmed_ids,
    infer_study_type,
    load_local_jcr_impact_factors,
    merge_rows,
...
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  <sheetData>
    <row r="1"><c r="A1" t="inlineStr"><is><t>JCR sample</t></is></c></row>
    <row r="3">
      <c r="A3" t="inlineStr"><is><t>序号</t></is></c>
...
class LedgerRulesTest(unittest.TestCase):
    ...
    def test_mesh_search_term_includes_heading_and_entry_terms(self):
        ...
        self.assertIn('"Arthritis, Rheumatoid"', term)
        self.assertIn('"Rheumatoid Arthritis"', term)

    def test_mesh_heading_matches_disease(self):
        self.assertTrue(strict_ra_match("Arthritis, Rheumatoid synovium")[0])
        self.assertTrue(strict_ra_match("Rheumatoid Arthritis synovium")[0])
    ...
    def test_score_rewards_clear_counts(self):
        ...
        self.assertNotIn("processed data likely available", reason)
    ...
    def test_removed_columns_are_not_in_output_fields(self):
        self.assertIn("manual_review", FIELDS)
        self.assertNotIn("gpl_titles", FIELDS)
        self.assertNotIn("processed_data_hint", FIELDS)

    def test_extract_source_pubmed_ids_from_soft(self):
        soft = "\n".join(
            [
                "^SERIES = GSE1",
                "!Series_pubmed_id = 12345",
                "!Series_pubmed_id = PMID: 67890; 12345",
                "^SAMPLE = GSM1",
                "!Series_pubmed_id = 99999",
            ]
        )
        self.assertEqual(extract_source_pubmed_ids(soft), ["12345", "67890"])

    def test_online_impact_factor_mapping_normalizes_journal_names(self):
        impact_factor = source_impact_factor(["ARTHRITIS & RHEUMATOLOGY (HOBOKEN, N.J.)."])
        self.assertIsNotNone(impact_factor)
        assert impact_factor is not None
        self.assertEqual(impact_factor.impact_factor, "10.9")
        self.assertEqual(impact_factor.year, "2024")

    def test_local_jcr_xlsx_mapping_normalizes_journal_names(self):
        with tempfile.TemporaryDirectory() as tmp:
            reference_dir = Path(tmp) / "IF reference"
            reference_dir.mkdir()
            write_sample_jcr_xlsx(reference_dir / "sample_jcr.xlsx")

            local_jcr = load_local_jcr_impact_factors(reference_dir)
            impact_factor = source_impact_factor(["Journal of translational medicine"], local_jcr=local_jcr)

        self.assertIsNotNone(impact_factor)
        assert impact_factor is not None
        self.assertEqual(impact_factor.impact_factor, "7.5")
        self.assertEqual(impact_factor.year, "2025")
        self.assertIn("local JCR XLSX", impact_factor.source)

    def test_row_without_source_pubmed_ids_leaves_source_fields_blank(self):
        record = GeoRecord(
            accession="GSE1",
            title="Rheumatoid arthritis blood RNA-seq",
            summary="Homo sapiens Expression profiling by high throughput sequencing processed counts synovium",
            organism="Homo sapiens",
            ...
            soft_text="\n".join(
                [
                    "^SERIES = GSE1",
                    "!Series_title = Rheumatoid arthritis blood RNA-seq",
                    "^SAMPLE = GSM1",
                    "!Sample_title = rheumatoid arthritis patient 1",
                    "^SAMPLE = GSM2",
                    "!Sample_title = rheumatoid arthritis patient 2",
        ...
        self.assertEqual(row["source_journal_impact_factor"], "")
        self.assertEqual(row["source_journal_impact_factor_year"], "")
    ...
    def test_row_writes_reuse_counts_and_impact_factor(self):
        ...
        row = row_for_record(
            ...
            reuse_counts=GseReuseCounts(total=5),
        )
        self.assertIsNotNone(row)
        assert row is not None
        self.assertEqual(row["source_journals"], "Arthritis & Rheumatology (Hoboken, N.J.)")
        self.assertEqual(row["source_journal_impact_factor"], "10.9")
        self.assertEqual(row["source_journal_impact_factor_year"], "2024")
    ...
    def test_enrich_existing_csv_files_updates_new_fields(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            csv_path.write_text("accession,title,score,sample_count\nGSE1,Title,100,RA=1; control=1; total=2\n", encoding="utf-8")

            with patch(
                "scripts.build_ra_geo_ledger.citation_fields_for_accession",
                return_value={
                    "source_journals": "Arthritis & Rheumatology (Hoboken, N.J.)",
            ...
            self.assertEqual(written[0], ",".join(FIELDS))
            self.assertIn("10.9", written[1])

    def test_static_impact_factor_mapping_does_not_query_online(self):
        with tempfile.TemporaryDirectory() as tmp:
            rows = [{"accession": "GSE1", "source_journals": "Arthritis & Rheumatology (Hoboken, N.J.)", "source_journal_impact_factor": "", "source_journal_impact_factor_year": ""}]
            with patch("scripts.build_ra_geo_ledger.lookup_online_impact_factor") as mocked:
                audit = apply_impact_factor_lookup(rows, cache_path=Path(tmp) / "cache.json", online_lookup=True, warnings=[])

            mocked.assert_not_called()
            self.assertEqual(rows[0]["source_journal_impact_factor"], "10.9")
            self.assertIn("Arthritis & Rheumatology (Hoboken, N.J.)", audit.static_matches)
    ...
    def test_online_impact_factor_lookup_updates_cache_and_row(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch(
                "scripts.build_ra_geo_ledger.lookup_online_impact_factor",
            ...
            self.assertIn("newjournal", cache_path.read_text(encoding="utf-8"))

    def test_cached_impact_factor_skips_online_lookup(self):
        with tempfile.TemporaryDirectory() as tmp:
            cache_path = Path(tmp) / "cache.json"
            cache_path.write_text(
                '{"version": 1, "journals": {"cachedjournal": {"journal": "Cached Journal", "impact_factor": "5.1", "year": "2025", "source": "cache", "source_url": "https://example.test"}}}',
                encoding="utf-8",
            )
            rows = [{"accession": "GSE3", "source_journals": "Cached Journal", "source_journal_impact_factor": "", "source_journal_impact_factor_year": ""}]
            with patch("scripts.build_ra_geo_ledger.lookup_online_impact_factor") as mocked:
                audit = apply_impact_factor_lookup(rows, cache_path=cache_path, online_lookup=True, warnings=[])

            mocked.assert_not_called()
            self.assertEqual(rows[0]["source_journal_impact_factor"], "5.1")
            self.assertIn("Cached Journal", audit.cache_matches)
    ...
    def test_missing_impact_factor_audit_skips_preprint_sources(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("scripts.build_ra_geo_ledger.lookup_online_impact_factor", return_value=None):
                audit = apply_impact_factor_lookup(rows, cache_path=Path(tmp) / "cache.json", online_lookup=True, warnings=[])
            ...
            self.assertIn("bioRxiv : the preprint server for biology", audit.skipped)
            self.assertIn("Unknown Journal", text)
