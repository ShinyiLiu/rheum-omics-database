#!/usr/bin/env python3
"""Build a GEO ledger for human primary Sjogren's syndrome expression transcriptome datasets."""
...
DEFAULT_OUT = ROOT / "data" / "pss_geo_transcriptome_datasets.csv"
DEFAULT_STATE = ROOT / "data" / "pss_geo_backfill_state.json"
DEFAULT_LATEST = ROOT / "data" / "pss_geo_latest_monthly_update.csv"
...
MESH_HEADING = "Sjogren's Syndrome"
MESH_ENTRY_TERMS = [
    "Sjogren Syndrome",
    "Sicca Syndrome",
]
...
PRIMARY_CONTEXT_TERMS = ["primary", "pss"]
SECONDARY_CONTEXT_TERMS = ["secondary sjogren", "secondary sjögren", "ra-associated", "rheumatoid arthritis-associated"]
...
TISSUE_TERMS = [
    ("salivary gland", ["salivary gland", "minor salivary", "labial salivary"]),
    ("lacrimal gland", ["lacrimal gland"]),
    ("blood", ["whole blood", "blood"]),
    ("PBMC", ["pbmc", "peripheral blood mononuclear"]),
    ("epithelial cell", ["epithelial cell", "salivary gland epithelial"]),
    ("B cell", ["b cell", "b-cell"]),
    ("T cell", ["t cell", "t-cell", "cd4", "cd8"]),
]


CLINICAL_TERMS = ["essdai", "focus score", "ssa", "ssb", "ro52", "ro60", "clinical", "activity", "response"]
TREATMENT_TERMS = ["treatment", "therapy", "drug", "response", "rituximab", "belimumab", "hydroxychloroquine", "steroid", "inhibitor"]
MIXED_DISEASE_TERMS = ["rheumatoid arthritis", "ankylosing spondylitis", "psoriasis", "psoriatic", "lupus", "systemic sclerosis", "osteoarthritis", "healthy and diseased"]
...
def add_ncbi_identity(params: dict[str, str]) -> dict[str, str]:
    ...
    enriched["tool"] = os.environ.get("NCBI_TOOL", "pss_geo_transcriptome_ledger")
...
def http_text(url: str, timeout: int = 60, retries: int = 3) -> str:
    req = urllib.request.Request(url, headers={"User-Agent": "codex-pss-geo-ledger/1.0"})
...
def monthly_archive_path(monthly_dir: Path, today: dt.date | None = None) -> Path:
    ...
    return monthly_dir / f"pss_geo_monthly_update_{stamp}.csv"
...
def strict_ra_match(text: str) -> tuple[bool, str]:
    folded = text.casefold()
    if any(term in folded for term in SECONDARY_CONTEXT_TERMS):
        return False, ""
    has_sjogren = any(term.casefold() in folded for term in [MESH_HEADING, *MESH_ENTRY_TERMS])
    has_primary_context = any(term in folded for term in PRIMARY_CONTEXT_TERMS)
    if has_sjogren and has_primary_context:
        for term in [MESH_HEADING, *MESH_ENTRY_TERMS, *CLINICAL_SEARCH_TERMS]:
            if term.casefold() in folded:
                return True, f"matched primary {term}"
        return True, "matched primary Sjogren's Syndrome"
    if re.search(r"(?<![a-z0-9])pss(?![a-z0-9])", folded):
        disease_context = any(token in folded for token in ["patient", "salivary", "lacrimal", "blood", "pbmc", "b cell", "t cell"])
        if disease_context:
            return True, "matched pSS in disease context"
    return False, ""
...
def classify_sample_group(text: str) -> str:
    ...
    if any(term in folded for term in SECONDARY_CONTEXT_TERMS):
        return "unclear"
    if "acr eular: positive" in folded:
        return "ra"
    if "acr eular: negative" in folded:
        return "control"
    has_sjogren = any(term.casefold() in folded for term in [MESH_HEADING, *MESH_ENTRY_TERMS])
    has_primary_context = any(term in folded for term in PRIMARY_CONTEXT_TERMS)
    if has_sjogren and has_primary_context:
...
def build_sample_count(samples: list[dict[str, list[str]]], total_hint: int | None, gse_text: str) -> SampleCounts:
    ...
    if samples:
        ...
        for sample in samples:
            ...
            if group == "ra":
                ...
                unclear += 1
        total = len(samples)
        parts = [f"pss={ra}", f"control={control}"]
        if unclear:
    ...
    if total is not None:
        return SampleCounts(f"pss=unknown; control=unknown; total={total}", "low", total, None, None, None, ["manual review needed for sample groups"])
    return SampleCounts("pss=unknown; control=unknown; total=unknown", "low", None, None, None, None, ["manual review needed for sample groups"])
...
def score_row(
    *,
    ...
    if counts.ra is not None and counts.ra > 0:
        score += 20
        reasons.append("pss samples clear")
    ...
    if counts.ra is not None and counts.control is not None and (counts.ra > 0 or counts.control > 0):
        score += 15
        reasons.append("pss/control counts shown")
    ...
    if counts.ra is None or counts.control is None or (counts.ra == 0 and counts.control == 0):
        score -= 20
        reasons.append("pss/control groups unclear")
    if counts.ra is not None and counts.ra == 0:
        score -= 20
        reasons.append("no pss samples parsed")
...
def validate_csv(path: Path) -> None:
    ...
    missing_counts = [row.get("accession", "") for row in rows if "pss=" not in row.get("sample_count", "") or "control=" not in row.get("sample_count", "")]
    if missing_counts:
        raise RuntimeError(f"sample_count missing pss/control format: {', '.join(missing_counts[:10])}")
