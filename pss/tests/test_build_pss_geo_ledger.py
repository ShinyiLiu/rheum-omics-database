...
from scripts.build_pss_geo_ledger import (
    FIELDS,
    GeoRecord,
    GseReuseCounts,
    JournalImpactFactor,
...
class LedgerRulesTest(unittest.TestCase):
    def test_strict_disease_match(self):
        self.assertTrue(strict_ra_match("primary Sjogren's syndrome patient blood RNA-seq")[0])
        self.assertFalse(strict_ra_match("unrelated healthy donor blood RNA-seq")[0])

    def test_sample_group_classification(self):
        self.assertEqual(classify_sample_group("primary Sjogren's syndrome patient sample"), "ra")
        self.assertEqual(classify_sample_group("healthy control PBMC"), "control")
        self.assertEqual(classify_sample_group("PBMC acr eular: Positive"), "ra")
        self.assertEqual(classify_sample_group("PBMC acr eular: Negative"), "control")

    def test_sample_count_format(self):
        samples = [
            {"!Sample_title": ["primary Sjogren's syndrome patient 1"]},
            {"!Sample_title": ["primary Sjogren's syndrome patient 2"]},
            {"!Sample_title": ["healthy control 1"]},
        ]
        counts = build_sample_count(samples, None, "")
        self.assertEqual(counts.sample_count, "pss=2; control=1; total=3")
        self.assertEqual(counts.confidence, "high")


    def test_mesh_search_term_includes_heading_and_primary_context(self):
        term = build_search_term()
        self.assertIn("\"primary Sjogren's syndrome\"", term)
        self.assertIn('"primary Sjogren syndrome"', term)
        self.assertIn('"primary Sicca Syndrome"', term)
        self.assertNotIn('(primary OR pSS)', term)

    def test_primary_scope_for_mesh_terms(self):
        self.assertTrue(strict_ra_match("primary Sjogren's Syndrome patient")[0])
        self.assertTrue(strict_ra_match("pSS patient salivary gland")[0])
        self.assertFalse(strict_ra_match("secondary Sjogren's Syndrome with rheumatoid arthritis")[0])
    ...
    def test_score_rewards_clear_counts(self):
        score, reason = score_row(
            text="primary Sjogren's syndrome healthy control clinical blood",
            organism="Homo sapiens",
            technology="bulk RNA-seq",
            counts=SampleCounts("pss=12; control=8; total=20", "high", 20, 12, 8, 0, []),
            tissue="blood",
            clinical_hint="yes",
        )
        self.assertEqual(score, 90)
        self.assertIn("pss/control counts shown", reason)

    def test_score_penalizes_no_parsed_cases(self):
        score, reason = score_row(
            text="primary Sjogren's syndrome blood",
            organism="Homo sapiens",
            technology="bulk RNA-seq",
            counts=SampleCounts("pss=0; control=0; other/unclear=8; total=8", "low", 8, 0, 0, 8, []),
            tissue="blood",
            clinical_hint="no",
        )
        self.assertLess(score, 50)
        self.assertIn("no pss samples parsed", reason)
    ...
    def test_monthly_archive_path(self):
        ...
        self.assertEqual(str(path).replace("\\", "/"), "data/monthly/pss_geo_monthly_update_2026-06.csv")
    ...
    def test_row_without_source_pubmed_ids_leaves_source_fields_blank(self):
        record = GeoRecord(
            ...
            soft_text="\n".join([
                "^SERIES = GSE1",
                "!Series_title = primary Sjogren's syndrome blood RNA-seq",
                "^SAMPLE = GSM1",
                "!Sample_title = primary Sjogren's syndrome patient 1",
                "^SAMPLE = GSM2",
                "!Sample_title = primary Sjogren's syndrome patient 2",
        ...
        self.assertIn("pss_relevance", row)
    ...
    def test_enrich_existing_csv_files_updates_new_fields(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            csv_path.write_text("accession,title,score,sample_count\nGSE1,Title,100,pss=1; control=1; total=2\n", encoding="utf-8")
            with patch(
                "scripts.build_pss_geo_ledger.citation_fields_for_accession",
                return_value={
    ...
    def test_online_impact_factor_lookup_updates_cache_and_row(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch(
                "scripts.build_pss_geo_ledger.lookup_online_impact_factor",
    ...
    def test_local_jcr_impact_factor_skips_online_lookup(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("scripts.build_pss_geo_ledger.lookup_online_impact_factor") as mocked:
    ...
    def test_missing_impact_factor_audit_skips_preprint_sources(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("scripts.build_pss_geo_ledger.lookup_online_impact_factor", return_value=None):
