#!/usr/bin/env python3
"""Build a GEO ledger for human ankylosing spondylitis expression transcriptome datasets."""
...
DEFAULT_OUT = ROOT / "data" / "as_geo_transcriptome_datasets.csv"
DEFAULT_STATE = ROOT / "data" / "as_geo_backfill_state.json"
DEFAULT_LATEST = ROOT / "data" / "as_geo_latest_monthly_update.csv"
...
MESH_HEADING = "Spondylitis, Ankylosing"
MESH_ENTRY_TERMS = [
...
TISSUE_TERMS = [
    ...
    ("enthesis", ["enthesis", "entheseal"]),
    ("T cell", ["t cell", "t-cell", "cd4", "cd8"]),
    ("B cell", ["b cell", "b-cell"]),
]


CLINICAL_TERMS = ["basdai", "basfi", "crp", "esr", "asdAS", "activity", "response", "clinical"]
TREATMENT_TERMS = ["treatment", "therapy", "drug", "response", "tnf", "anti-tnf", "il-17", "secukinumab", "adalimumab", "etanercept", "infliximab", "jak", "inhibitor"]
MIXED_DISEASE_TERMS = ["rheumatoid arthritis", "psoriasis", "psoriatic", "lupus", "systemic sclerosis", "sjogren", "osteoarthritis", "healthy and diseased"]
...
def add_ncbi_identity(params: dict[str, str]) -> dict[str, str]:
    ...
    enriched["tool"] = os.environ.get("NCBI_TOOL", "as_geo_transcriptome_ledger")
...
def http_text(url: str, timeout: int = 60, retries: int = 3) -> str:
    req = urllib.request.Request(url, headers={"User-Agent": "codex-as-geo-ledger/1.0"})
...
def monthly_archive_path(monthly_dir: Path, today: dt.date | None = None) -> Path:
    ...
    return monthly_dir / f"as_geo_monthly_update_{stamp}.csv"
...
def classify_sample_group(text: str) -> str:
    folded = text.casefold()
    if any(term.casefold() in folded for term in RA_TERMS):
        return "ra"
    control_patterns = [
        "healthy control",
        "healthy controls",
...
def build_sample_count(samples: list[dict[str, list[str]]], total_hint: int | None, gse_text: str) -> SampleCounts:
    ...
    if samples:
        ...
        for sample in samples:
            ...
            if group == "ra":
                ...
                unclear += 1
        total = len(samples)
        parts = [f"AS={ra}", f"control={control}"]
        if unclear:
    ...
    if total is not None:
        return SampleCounts(f"AS=unknown; control=unknown; total={total}", "low", total, None, None, None, ["manual review needed for sample groups"])
    return SampleCounts("AS=unknown; control=unknown; total=unknown", "low", None, None, None, None, ["manual review needed for sample groups"])
...
def score_row(
    *,
    ...
    if counts.ra is not None and counts.ra > 0:
        score += 20
        reasons.append("AS samples clear")
    ...
    if counts.ra is not None and counts.control is not None and (counts.ra > 0 or counts.control > 0):
        score += 15
        reasons.append("AS/control counts shown")
    ...
    if counts.ra is None or counts.control is None or (counts.ra == 0 and counts.control == 0):
        score -= 20
        reasons.append("AS/control groups unclear")
    if counts.ra is not None and counts.ra == 0:
        score -= 20
        reasons.append("no AS samples parsed")
...
def build_search_term() -> str:
    disease = quoted_search_terms([MESH_HEADING, *MESH_ENTRY_TERMS, *CLINICAL_SEARCH_TERMS])
...
def validate_csv(path: Path) -> None:
    ...
    missing_counts = [row.get("accession", "") for row in rows if "AS=" not in row.get("sample_count", "") or "control=" not in row.get("sample_count", "")]
    if missing_counts:
        raise RuntimeError(f"sample_count missing AS/control format: {', '.join(missing_counts[:10])}")
