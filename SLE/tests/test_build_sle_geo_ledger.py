...
from scripts.build_sle_geo_ledger import (
    FIELDS,
    GeoRecord,
    GseReuseCounts,
    JournalImpactFactor,
...
class LedgerRulesTest(unittest.TestCase):
    def test_strict_disease_match(self):
        self.assertTrue(strict_ra_match("systemic lupus erythematosus patient blood RNA-seq")[0])
        self.assertFalse(strict_ra_match("unrelated healthy donor blood RNA-seq")[0])

    def test_sample_group_classification(self):
        self.assertEqual(classify_sample_group("systemic lupus erythematosus patient sample"), "ra")
        self.assertEqual(classify_sample_group("healthy control PBMC"), "control")

    def test_sample_count_format(self):
        samples = [
            {"!Sample_title": ["systemic lupus erythematosus patient 1"]},
            {"!Sample_title": ["systemic lupus erythematosus patient 2"]},
            {"!Sample_title": ["healthy control 1"]},
        ]
        counts = build_sample_count(samples, None, "")
        self.assertEqual(counts.sample_count, "SLE=2; control=1; total=3")
        self.assertEqual(counts.confidence, "high")


    def test_mesh_search_term_includes_sle_and_lupus_nephritis_terms(self):
        term = build_search_term()
        self.assertIn('"Lupus Erythematosus, Systemic"', term)
        self.assertIn('"Systemic Lupus Erythematosus"', term)
        self.assertIn('"Lupus Nephritis"', term)
        self.assertIn('"Nephritis, Lupus"', term)

    def test_mesh_heading_and_lupus_nephritis_entry_terms_match(self):
        self.assertTrue(strict_ra_match("Lupus Erythematosus, Systemic patient")[0])
        self.assertTrue(strict_ra_match("Nephritis, Lupus kidney biopsy")[0])
    ...
    def test_score_rewards_clear_counts(self):
        score, reason = score_row(
            text="systemic lupus erythematosus healthy control clinical blood",
            organism="Homo sapiens",
            technology="bulk RNA-seq",
            counts=SampleCounts("SLE=12; control=8; total=20", "high", 20, 12, 8, 0, []),
            tissue="blood",
            clinical_hint="yes",
        )
        self.assertEqual(score, 90)
        self.assertIn("SLE/control counts shown", reason)

    def test_score_penalizes_no_parsed_cases(self):
        score, reason = score_row(
            text="systemic lupus erythematosus blood",
            organism="Homo sapiens",
            technology="bulk RNA-seq",
            counts=SampleCounts("SLE=0; control=0; other/unclear=8; total=8", "low", 8, 0, 0, 8, []),
            tissue="blood",
            clinical_hint="no",
        )
        self.assertLess(score, 50)
        self.assertIn("no SLE samples parsed", reason)
    ...
    def test_monthly_archive_path(self):
        ...
        self.assertEqual(str(path).replace("\\", "/"), "data/monthly/sle_geo_monthly_update_2026-06.csv")
    ...
    def test_row_without_source_pubmed_ids_leaves_source_fields_blank(self):
        record = GeoRecord(
            ...
            soft_text="\n".join([
                "^SERIES = GSE1",
                "!Series_title = systemic lupus erythematosus blood RNA-seq",
                "^SAMPLE = GSM1",
                "!Sample_title = systemic lupus erythematosus patient 1",
                "^SAMPLE = GSM2",
                "!Sample_title = systemic lupus erythematosus patient 2",
        ...
        self.assertIn("sle_relevance", row)
    ...
    def test_enrich_existing_csv_files_updates_new_fields(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            csv_path.write_text("accession,title,score,sample_count\nGSE1,Title,100,SLE=1; control=1; total=2\n", encoding="utf-8")
            with patch(
                "scripts.build_sle_geo_ledger.citation_fields_for_accession",
                return_value={
    ...
    def test_online_impact_factor_lookup_updates_cache_and_row(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch(
                "scripts.build_sle_geo_ledger.lookup_online_impact_factor",
    ...
    def test_local_jcr_impact_factor_skips_online_lookup(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("scripts.build_sle_geo_ledger.lookup_online_impact_factor") as mocked:
    ...
    def test_missing_impact_factor_audit_skips_preprint_sources(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("scripts.build_sle_geo_ledger.lookup_online_impact_factor", return_value=None):
    ...
    def test_lupus_nephritis_is_sle_case(self):
        self.assertTrue(strict_ra_match("lupus nephritis patient kidney biopsy")[0])
        self.assertEqual(classify_sample_group("LN patient kidney biopsy"), "ra")
        self.assertEqual(classify_sample_group("LN technical replicate"), "unclear")
