#!/usr/bin/env python3
"""Build a GEO ledger for human systemic lupus erythematosus and lupus nephritis expression transcriptome datasets."""
...
DEFAULT_OUT = ROOT / "data" / "sle_geo_transcriptome_datasets.csv"
DEFAULT_STATE = ROOT / "data" / "sle_geo_backfill_state.json"
DEFAULT_LATEST = ROOT / "data" / "sle_geo_latest_monthly_update.csv"
...
MESH_HEADING = "Lupus Erythematosus, Systemic"
MESH_ENTRY_TERMS = [
    "Systemic Lupus Erythematosus",
    "Lupus Erythematosus Disseminatus",
    "Libman-Sacks Disease",
]
LUPUS_NEPHRITIS_MESH_HEADING = "Lupus Nephritis"
LUPUS_NEPHRITIS_ENTRY_TERMS = [
...
TISSUE_TERMS = [
    ...
    ("kidney", ["kidney", "renal", "glomerulus", "glomeruli", "tubulointerstitium"]),
    ("urine", ["urine", "urinary"]),
...
CLINICAL_TERMS = ["sledai", "bilag", "flare", "nephritis", "proteinuria", "anti-dsdna", "complement", "clinical", "activity", "response"]
TREATMENT_TERMS = ["treatment", "therapy", "drug", "response", "steroid", "glucocorticoid", "mycophenolate", "cyclophosphamide", "rituximab", "belimumab", "hydroxychloroquine", "inhibitor"]
MIXED_DISEASE_TERMS = ["rheumatoid arthritis", "ankylosing spondylitis", "psoriasis", "psoriatic", "systemic sclerosis", "sjogren", "osteoarthritis", "healthy and diseased"]
...
def add_ncbi_identity(params: dict[str, str]) -> dict[str, str]:
    ...
    enriched["tool"] = os.environ.get("NCBI_TOOL", "sle_geo_transcriptome_ledger")
...
def http_text(url: str, timeout: int = 60, retries: int = 3) -> str:
    req = urllib.request.Request(url, headers={"User-Agent": "codex-sle-geo-ledger/1.0"})
...
def monthly_archive_path(monthly_dir: Path, today: dt.date | None = None) -> Path:
    ...
    return monthly_dir / f"sle_geo_monthly_update_{stamp}.csv"
...
def strict_ra_match(text: str) -> tuple[bool, str]:
    ...
    for term in RA_TERMS:
        ...
        if term_folded == "sle":
            if re.search(r"(?<![a-z0-9])sle(?![a-z0-9])", folded):
                disease_context = any(token in folded for token in ["patient", "blood", "pbmc", "kidney", "renal", "nephritis", "lupus"])
                if disease_context:
                    return True, "matched SLE in disease context"
        elif term_folded == "ln":
            if re.search(r"(?<![a-z0-9])ln(?![a-z0-9])", folded):
                disease_context = any(token in folded for token in ["patient", "kidney", "renal", "nephritis", "lupus", "biopsy"])
                if disease_context:
                    return True, "matched LN in lupus nephritis context"
...
def classify_sample_group(text: str) -> str:
    folded = text.casefold()
    if any(term.casefold() in folded for term in RA_TERMS if term.casefold() not in {"sle", "ln"}):
        return "ra"
    if re.search(r"(?<![a-z0-9])sle(?![a-z0-9])", folded):
        disease_context = any(token in folded for token in ["patient", "blood", "pbmc", "kidney", "renal", "nephritis", "lupus"])
        if disease_context:
...
def build_sample_count(samples: list[dict[str, list[str]]], total_hint: int | None, gse_text: str) -> SampleCounts:
    ...
    if samples:
        ...
        for sample in samples:
            ...
            if group == "ra":
                ...
                unclear += 1
        total = len(samples)
        parts = [f"SLE={ra}", f"control={control}"]
        if unclear:
    ...
    if total is not None:
        return SampleCounts(f"SLE=unknown; control=unknown; total={total}", "low", total, None, None, None, ["manual review needed for sample groups"])
    return SampleCounts("SLE=unknown; control=unknown; total=unknown", "low", None, None, None, None, ["manual review needed for sample groups"])
...
def score_row(
    *,
    ...
    if counts.ra is not None and counts.ra > 0:
        score += 20
        reasons.append("SLE samples clear")
    ...
    if counts.ra is not None and counts.control is not None and (counts.ra > 0 or counts.control > 0):
        score += 15
        reasons.append("SLE/control counts shown")
    ...
    if counts.ra is None or counts.control is None or (counts.ra == 0 and counts.control == 0):
        score -= 20
        reasons.append("SLE/control groups unclear")
    if counts.ra is not None and counts.ra == 0:
        score -= 20
        reasons.append("no SLE samples parsed")
...
def validate_csv(path: Path) -> None:
    ...
    missing_counts = [row.get("accession", "") for row in rows if "SLE=" not in row.get("sample_count", "") or "control=" not in row.get("sample_count", "")]
    if missing_counts:
        raise RuntimeError(f"sample_count missing SLE/control format: {', '.join(missing_counts[:10])}")
